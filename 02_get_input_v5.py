...
class NewJob:
    def __init__(self):
        ...
        # set up error label
        self.error_label = Label(self.input_frame, text="")
        self.error_label.grid(row=8, columnspan=2)
    ...
    def store_info(self):
        ...
        if self.customer_name.get() == "" or self.distance.get() == "" or self.virus_time.get() == "":
            self.error_label.configure(text="Please do not leave any of the entries blank.")
        else:
            customer_name = self.customer_name.get()
            try:
                distance = float(self.distance.get())
            except ValueError:
                self.error_label.configure(text="Distance should be a number above 0")
            else:
                # check virus time entry is a number
                try:
                    virus_time = float(self.virus_time.get())
                except ValueError:
                    self.error_label.configure(text="Virus Time should be a number above or equal to 0")
                else:
